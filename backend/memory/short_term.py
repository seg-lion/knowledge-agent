'''
短期记忆模块————管理当前会话的所有上下文，会话结束时触发摘要写入长期记忆
...
class ShortTermMemory:
    ...
    def __init__(self, session_id: str):
        ...
        self.messages: deque[dict] = deque(maxlen = settings.short_term_max_rounds) # 是一个固定容量的双端队列。消息超过maxlen时，最旧的自动被淘汰
        self.retrieved_docs: list[str] = [] # 本次会话检索过的chunk_id
        self.accessed_topics: dict[str, int] ={}    # topic -> 访问次数
    ...
    def add_message(self, role: str, content: str) -> None:
        ...
        self.messages.append(
            {
                "role": role,
                "content": content[:2000],  # 阶段长消息(这里指的是字符吧)
